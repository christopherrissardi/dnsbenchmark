...
DATABASE_PATH = os.path.join("data", "DNS_Servers.db")
...
def get_top_dns():
    """Busca todos os servidores da tabela tops_dns (Lista de Elite)."""
    conn = _get_db_connection()
    if not conn: return []
    
    cursor = conn.cursor()
    # Removemos o filtro is_top, pois a tabela já é a lista correta
    cursor.execute("SELECT * FROM tops_dns")
...
def get_dns_by_city(city):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM tops_dns WHERE lower(city) = ?", (city.lower(),))
    servers = cursor.fetchall()
    conn.close()
    return servers
...
def get_dns_by_country(country_id):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM tops_dns WHERE country_id = ?", (country_id.upper(),))
    servers = cursor.fetchall()
    conn.close()
    return servers
...
def get_all_dns_counts(city, country_id):
    ...
    # Contagem Total (Tops)
    cursor.execute("SELECT COUNT(ip) AS total FROM tops_dns")
    top_dns_count = cursor.fetchone()['total']
    
    # Contagem Cidade
    cursor.execute("SELECT COUNT(ip) AS total FROM tops_dns WHERE lower(city) = ?", (city.lower(),))
    city_dns_count = cursor.fetchone()['total']
    
    # Contagem País
    cursor.execute("SELECT COUNT(ip) AS total FROM tops_dns WHERE country_id = ?", (country_id.upper(),))
